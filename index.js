...
function channelWithMostContent(channels) {
  ...
  channels.forEach((channel) =>
    videoDurations.push([channel, totalVideosDuration(channel)])
  );
  videoDurations.sort((a, b) => {
    return b[1] - a[1];
  });
  const highestDuration = videoDurations[0][0];
...
function longestChannelName(channels) {
  ...
  channelNameLengths = [];
  channels.forEach((channel) =>
    channelNameLengths.push([channel, channel.name.length])
  );
  channelNameLengths.sort((a, b) => {
    return b[1] - a[1];
  });
  const longestName = channelNameLengths[0][0];
  return longestName;
}
